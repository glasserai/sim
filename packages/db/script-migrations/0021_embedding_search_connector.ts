...
/**
 * The document trigger's body. A document whose source or ACL changed is marked for the knowledge
 * projector, whatever the writer's mode; a writer that did not declare the asynchronous mode also
 * fans the new values out to its enabled chunks in its own statement, as every writer did before
 * the projector existed.
 *
 * The fan-out leaves a chunk the source and ACL fill has not reached (`acl IS NULL`) without an
 * ACL: search decides such a row on its document, so writing it there changes no answer, while
 * every write to `embedding_search` re-inserts the row into its vector index. A document that moves
 * to another source still carries the source onto those chunks, because source filters read it from
 * the row. The projector fills them from the document once the mark reaches it.
 */
export async function replaceProjectionSourceAclSync(sql: Sql | TransactionSql): Promise<void> {
  ...
  await sql.unsafe(`CREATE OR REPLACE FUNCTION sync_projection_source_acl()
    ...
    BEGIN
      ...
      END IF;
      IF current_setting('${KNOWLEDGE_PROJECTION_MODE_SETTING}', true) = '${KNOWLEDGE_PROJECTION_ASYNC}' THEN
        RETURN NEW;
      END IF;${PROJECTION_SOURCE_ACL_TABLES.map(fanOut).join('')}
      RETURN NEW;
    END;
    $$`)
}
