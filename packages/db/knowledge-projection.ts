 ...
 */
export const KNOWLEDGE_PROJECTION_MODE_SETTING = 'sim.projection_mode'

/** The value of {@link KNOWLEDGE_PROJECTION_MODE_SETTING} that defers projection to the projector. */
export const KNOWLEDGE_PROJECTION_ASYNC = 'async'

/**
 * The `set_config` fragment a knowledge writer runs in its transaction to declare the projection
 * mode; `async` leaves its search projection rows to the projector. Transaction-local, so a pooled
 * connection never carries it into the next transaction. Writers resolve the mode from the
 * `knowledge-async-projection` flag on their own write path.
 */
export function knowledgeProjectionModeSetting(async: boolean): SQL {
  return sql`set_config(${KNOWLEDGE_PROJECTION_MODE_SETTING}, ${async ? KNOWLEDGE_PROJECTION_ASYNC : 'sync'}, true)`
}

/**
 * The `WHEN` clause of every trigger that writes projection rows in the writer's transaction: it
 * fires unless the transaction declared the asynchronous mode.
 */
export const SYNCHRONOUS_PROJECTION_WHEN = `current_setting('${KNOWLEDGE_PROJECTION_MODE_SETTING}', true) IS DISTINCT FROM '${KNOWLEDGE_PROJECTION_ASYNC}'`
...
/** The projections that mirror their document's source and ACL. */
const SOURCE_ACL_PROJECTIONS: ReadonlySet<KnowledgeProjection> = new Set([
  'embedding_search',
  'embedding_keyword_tin',
])
...
async function enterProjectorTransaction(tx: TransactionSql, lockTimeoutMs: number) {
  await tx.unsafe(
    `SELECT set_config('lock_timeout', '${lockTimeoutMs}ms', true),
      ...
      set_config('${KNOWLEDGE_PROJECTION_MODE_SETTING}', '${KNOWLEDGE_PROJECTION_ASYNC}', true)`
  )
}
...
async function projectMarkedDocument(
  ...
  try {
    ...
    for (const projection of projections) {
      if (!mark.content && !SOURCE_ACL_PROJECTIONS.has(projection)) continue
...
/** The projections whose rows carry a source and ACL the fill may not have reached. */
const FILLED_PROJECTIONS = ['embedding_search', 'embedding_keyword_tin'] as const
 ...
 * a row whose document is gone is passed over. Returns how many documents were marked and the id
 * to continue after, or `null` once every projection's unfilled rows have been read.
 */
export async function markUnfilledProjectionDocuments(
  ...
  for (let index = cursor.projection; index < FILLED_PROJECTIONS.length; index++) {
    const projection = FILLED_PROJECTIONS[index]
    ...
    const [row] = await sql.unsafe<Array<{ marked: number; last_id: string | null }>>(
      `WITH unfilled AS MATERIALIZED (
        ...
        SELECT DISTINCT u.document_id FROM unfilled u JOIN document d ON d.id = u.document_id
        ORDER BY u.document_id LIMIT $2
      ), marked AS (
      ...
      ) SELECT (SELECT count(*)::int FROM marked) AS marked, (SELECT max(id) FROM unfilled) AS last_id`,
